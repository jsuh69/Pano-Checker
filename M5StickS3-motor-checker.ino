...
/** 신호 검출 중 Hz/RPM 표시 갱신(FFT는 마이크 버퍼마다 수행, 여기서는 LCD만
 * 스로틀). */
...
ArduinoFFT<double> FFT = ArduinoFFT<double>(vReal, vImag, Audio::kSamples,
                                            Audio::kSamplingFrequency);
...
private:
  ...
  bool externalPowerOrCharging() {
    ...
    if (M5.Power.isCharging() == chg_t::is_charging)
      return true;
...
public:
  ...
  void update() {
    ...
    if (onExternalPower) {
      ...
      if (now - lastChargingPrecisionMs >=
          BatConfig::kPrecisionSamplePeriodMs) {
        M5.Power.setBatteryCharge(false);
      ...
      } else if (now - lastUnpluggedSampleMs >=
                 BatConfig::kUnpluggedSamplePeriodMs) {
        stableBatPct = clampBatteryPercent(M5.Power.getBatteryLevel());
        lastUnpluggedSampleMs = now;
      }
    }
...
void updatePowerSaveState(bool rpmActive) {
  if (rpmActive) {
    ...
    if (sReadySinceMs == 0)
      sReadySinceMs = millis();
    if (!sDisplayBlanked &&
        (millis() - sReadySinceMs >= PowerSave::kDisplayBlankAfterReadyMs)) {
...
void updateDisplay(double currentHz, bool isDetected, bool rpmActive) {
  ...
  const unsigned long uiPeriod =
      isDetected ? PowerSave::kUiRefreshActiveMs : PowerSave::kUiRefreshIdleMs;
    ...
    if (drawToPanel) {
      ...
      uint16_t batColor =
          batteryManager.isCharging()
              ? UI::COLOR_BAT_CHARGING
              : (batteryManager.getBatteryPercent() > 20 ? UI::COLOR_BAT_GOOD
                                                         : UI::COLOR_BAT_LOW);
      spr.setTextColor(batColor);
      spr.setFont(&fonts::FreeSans9pt7b);
      spr.drawRightString(String(batteryManager.getBatteryPercent()) + "%",
                          spr.width() - 5, 12);
